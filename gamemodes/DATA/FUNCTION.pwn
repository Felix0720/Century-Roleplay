...
function OnPlayerDataLoaded(playerid, race_check)
{
	...
	if(cache_num_rows() > 0)
	{
		if(pData[playerid][pPassword] < 1)
		{
			...
			Dialog_Show(playerid, VerifyCodeScreen, DIALOG_STYLE_INPUT, "Verify Account", str, "Input", "Cancel");
		}
		if(pData[playerid][pPassword] > 10)
		{
			...
			Dialog_Show(playerid, LoginScreen, DIALOG_STYLE_PASSWORD, "Login to Zone City", lstring, "Login", "Quit");
		}
	}
	else
	{
		Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX,"Account Unregistered","{FFFF00}Nama Akun ini belum terdaftar!\n{ffffff}Silahkan daftar di Official Discord kami {15D4ED}Century City Roleplay\n{ffffff}Link Discord: {15D4ED}https://discord.gg/QpSDsAKgtB","Okay","");
	...
	if(cache_num_rows() > 0)
	{
		...
		else
		{
			...
			if(banTime_Int == 0)
			{
				...
				Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, "{FFFFFF}Banned Alert - UCP Ban", lstr, "Close", "");
			}
			else
			{
				new lstr[512];
	...
	if(cache_num_rows() > 0)
	{
		...
		else
		{
			...
			if(banTime_Int == 0)
			{
				...
				Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, ""RED_E"BANNED", lstr, "Exit", "");
			}
			else
			{
				new lstr[512];
...
{
	new aname[MAX_PLAYER_NAME], name[MAX_PLAYER_NAME], ucp[22], email[40], age[128], ip[128], regdate[50], lastlogin[50];
...
function CheckDoubleName(playerid, name[])
{
	new rows = cache_num_rows();
	if(rows > 0)
	{
		Dialog_Show(playerid, MakeCharScreen, DIALOG_STYLE_INPUT, "Create Character", "ERROR: This name is already used by the other player!\nInsert your new Character Name\n\nExample: Finn_Xanderz, Javier_Cooper etc.", "Create", "Back");
	}
	else 
	{
...
function OnLoginTimeout(playerid)
{
	...
	Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, "Login", "You have been kicked for taking too long to login successfully to your account.", "Okay", "");
	KickEx(playerid);
	return 1;
}
...
function LoadStats(playerid, PlayersName[])
{
	...
	else
	{
		...
		Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, header, gstr, "Close", "");
...
function CheckPlayerIP(playerid, zplayerIP[])
{
	...
	if(count)
	{
		...
		Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, tstr, line, "Close", "");
	}
	else
 	{
...
{
	Dialog_Show(playerid, -1, DIALOG_STYLE_LIST, "Close", "Close", "Close", "Close");
...
function SearchRek2(playerid, rek)
{
	if(cache_num_rows())
	{
		...
		Dialog_Show(playerid, BANKConfirm, DIALOG_STYLE_MSGBOX, ""LB_E"Bank", mstr, "Transfer", "Cancel");
	}
	return true;
}
