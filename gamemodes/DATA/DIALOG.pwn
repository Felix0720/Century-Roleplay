...
Dialog:UnusedScreen(playerid, response, listitem, inputtext[])
{

	return 1;
}

Dialog:LoginScreen(playerid, response, listitem, inputtext[], dialogid)
{
	...
	else
	{
		...
		{
			Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, "Login", "You have mistyped your password too often (3 times).", "Okay", "");
			KickEx(playerid);
		}
		else 
		{
			new lstring[300];
			format(lstring, sizeof lstring, "{ffffff}Welcome back to {15D4ED}Century City Roleplay\n{ffffff}Account: {15D4ED}%s\n{ffffff}Attemps: {FF0000}%i/3{ffffff}\nPassword: {3BBD44}(input below)\n\n{FF0000}Wrong password!", pData[playerid][pUCP], pData[playerid][LoginAttempts]);
			Dialog_Show(playerid, LoginScreen, DIALOG_STYLE_PASSWORD, "Login to IndoLast", lstring, "Login", "Quit");
		}
	}
	return 1;
}
...
Dialog:RegisterScreen(playerid, response, listitem, inputtext[])
{
	...
	if (strlen(inputtext) <= 5) return Dialog_Show(playerid, RegisterScreen, DIALOG_STYLE_PASSWORD, "Registration IndoLast", "Kata sandi minimal 5 Karakter!\nMohon isi Password dibawah ini:", "Register", "Tolak");
	
	if(!IsValidPassword(inputtext))
	{
		Error(playerid, "Sandi valid : A-Z, a-z, 0-9, _, [ ], ( )");
		Dialog_Show(playerid, RegisterScreen, DIALOG_STYLE_PASSWORD, "Registration IndoLast", "Kata sandi yang anda gunakan mengandung karakter yang valid!\nMohon isi Password dibawah ini:", "Register", "Tolak");
		return 1;
	}
...
Dialog:CharListScreen(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		if(PlayerChar[playerid][listitem][0] == EOS)
			return Dialog_Show(playerid, MakeCharScreen, DIALOG_STYLE_INPUT, "Create Character", "Insert your new Character Name\n\nExample: Finn_Xanderz, Javier_Cooper etc.", "Create", "Exit");
...
Dialog:MakeCharScreen(playerid, response, listitem, inputtext[])
{
	if(response)
    {
		...
		mysql_tquery(g_SQL, characterQuery, "CheckDoubleName", "ds", playerid, inputtext);
	    format(pData[playerid][pUCP], 22, GetName(playerid));
	}
	return 1;
}

Dialog:AgeScreen(playerid, response, listitem, inputtext[])
{
	if(!response) return Dialog_Show(playerid, AgeScreen, DIALOG_STYLE_INPUT, "Tanggal Lahir", "Masukan tanggal lahir (Tgl/Bulan/Tahun): 15/04/1998", "Pilih", "Batal");
	if(response)
	{
		new
			iDay,
		...
		if(sscanf(inputtext, "p</>ddd", iDay, iMonth, iYear)) {
			Dialog_Show(playerid, AgeScreen, DIALOG_STYLE_INPUT, "Tanggal Lahir", "Error! Invalid Input\nMasukan tanggal lahir (Tgl/Bulan/Tahun): 15/04/1998", "Pilih", "Batal");
		}
		else if(iYear < 1900 || iYear > year) {
			Dialog_Show(playerid, AgeScreen, DIALOG_STYLE_INPUT, "Tahun Lahir", "Error! Invalid Input\nMasukan tanggal lahir (Tgl/Bulan/Tahun): 15/04/1998", "Pilih", "Batal");
		}
		else if(iMonth < 1 || iMonth > 12) {
			Dialog_Show(playerid, AgeScreen, DIALOG_STYLE_INPUT, "Bulan Lahir", "Error! Invalid Input\nMasukan tanggal lahir (Tgl/Bulan/Tahun): 15/04/1998", "Pilih", "Batal");
		}
		...
		else 
		{
			...
			Dialog_Show(playerid, GenderScreen, DIALOG_STYLE_LIST, "Gender", "1. Male/Laki-Laki\n2. Female/Perempuan", "Pilih", "Batal");
		}
	}
	else Dialog_Show(playerid, AgeScreen, DIALOG_STYLE_INPUT, "Tanggal Lahir", "Masukan tanggal lahir (Tgl/Bulan/Tahun): 15/04/1998", "Pilih", "Batal");
	return 1;
}

Dialog:GenderScreen(playerid, response, listitem, inputtext[])
{
    	if(!response) return Dialog_Show(playerid, GenderScreen, DIALOG_STYLE_LIST, "Gender", "1. Male/Laki-Laki\n2. Female/Perempuan", "Pilih", "Batal");
		if(response)
		{
		...
		else Dialog_Show(playerid, GenderScreen, DIALOG_STYLE_LIST, "Gender", "1. Male/Laki-Laki\n2. Female/Perempuan", "Pilih", "Batal");
		return 1;
}

Dialog:EmailScreen(playerid, response, listitem, inputtext[])
{
    if(response)
	{
		if(isnull(inputtext))
...
Dialog:PasswordScreen(playerid, response, listitem, inputtext[])
{
    if(response)
	{
...
Dialog:StatsScreen(playerid, response, listitem, inputtext[])
{
	if(response)
	{
...
Dialog:SettingsScreen(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
		{
			...
			case 2:
			{
				Dialog_Show(playerid, HBEMODE, DIALOG_STYLE_LIST, "HBE Mode", ""LG_E"Simple\n"LG_E"Modern\n"RED_E"Disable", "Set", "Close");
			}
			case 3:
			{
...
Dialog:HBEMODE(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:ChangeAge(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		new
		...
		if(sscanf(inputtext, "p</>ddd", iDay, iMonth, iYear)) {
			Dialog_Show(playerid, ChangeAge, DIALOG_STYLE_INPUT, "Tanggal Lahir", "Error! Invalid Input\nMasukan tanggal lahir (Tgl/Bulan/Tahun): 15/04/1998", "Pilih", "Batal");
		}
		else if(iYear < 1900 || iYear > year) {
			Dialog_Show(playerid, ChangeAge, DIALOG_STYLE_INPUT, "Tahun Lahir", "Error! Invalid Input\nMasukan tanggal lahir (Tgl/Bulan/Tahun): 15/04/1998", "Pilih", "Batal");
		}
		else if(iMonth < 1 || iMonth > 12) {
			Dialog_Show(playerid, ChangeAge, DIALOG_STYLE_INPUT, "Bulan Lahir", "Error! Invalid Input\nMasukan tanggal lahir (Tgl/Bulan/Tahun): 15/04/1998", "Pilih", "Batal");
		}
		else if(iDay < 1 || iDay > arrMonthDays[iMonth - 1]) {
			Dialog_Show(playerid, ChangeAge, DIALOG_STYLE_INPUT, "Tanggal Lahir", "Error! Invalid Input\nMasukan tanggal lahir (Tgl/Bulan/Tahun): 15/04/1998", "Pilih", "Batal");
		}
...
Dialog:GoldShop(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
		{
			case 0:
			{
				...
				Dialog_Show(playerid, GoldName, DIALOG_STYLE_INPUT, "Change Name", "Input new nickname:\nExample: Charles_Sanders\n", "Change", "Cancel");
			}
			case 1:
			{
...
Dialog:GoldName(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		if(strlen(inputtext) < 4) return Error(playerid, "New name can't be shorter than 4 characters!"),  Dialog_Show(playerid, GoldName, DIALOG_STYLE_INPUT, ""WHITE_E"Change Name", "Enter your new name:", "Enter", "Exit");
		if(strlen(inputtext) > 20) return Error(playerid, "New name can't be longer than 20 characters!"),  Dialog_Show(playerid, GoldName, DIALOG_STYLE_INPUT, ""WHITE_E"Change Name", "Enter your new name:", "Enter", "Exit");
		if(!IsValidRoleplayName(inputtext))
		{
			Error(playerid, "Name contains invalid characters, please doublecheck!");
			Dialog_Show(playerid, GoldName, DIALOG_STYLE_INPUT, ""WHITE_E"Change Name", "Enter your new name:", "Enter", "Exit");
			return 1;
		}
		new query[512];
...
Dialog:SellBisniss(playerid, response, listitem, inputtext[])
{
	...
	Dialog_Show(playerid, SellBisnis, DIALOG_STYLE_MSGBOX, "Sell Bisnis", str, "Sell", "Cancel");
	return 1;
}
...
Dialog:MyBisnis(playerid, response, listitem, inputtext[])
{
	...
	Dialog_Show(playerid, BisnisInfo, DIALOG_STYLE_LIST, "{FF0000}SoI:RP {0000FF}Bisnis", "Show Information\nTrack Bisnis", "Select", "Cancel");
	return 1;
}
...
Dialog:BisnisInfo(playerid, response, listitem, inputtext[])
{
	...
	switch(listitem)
	{
		case 0:
		{
			...
			Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, "Bisnis Info", line9, "Close","");
		}
		case 1:
		{
...
Dialog:BisnisMenu(playerid, response, listitem, inputtext[])
{
	...
	if(response)
	{
		switch(listitem)
		{
			...
			{	
				...
				Dialog_Show(playerid, BisnisInfo, DIALOG_STYLE_TABLIST, mstr, lstr,"Back","Close");
			}
			case 1:
			{
				new mstr[248];
			...
			case 2: Dialog_Show(playerid, BisnisVault, DIALOG_STYLE_LIST,"Bisnis Vault","Deposit\nWithdraw","Next","Back");
			case 3:
			{
...
Dialog:InfoBisnis(playerid, response, listitem, inputtext[])
{
	if(response)
	{
...
Dialog:BisnisVault(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
		{
			case 0:
			{
				...
				Dialog_Show(playerid, BisnisDeposit, DIALOG_STYLE_INPUT, "Deposit", mstr, "Deposit", "Back");
			}
			case 1:
			{
				new mstr[512];
				...
				Dialog_Show(playerid, BisnisWithdraw, DIALOG_STYLE_INPUT,"Withdraw", mstr,"Withdraw","Back");
			}
		}
	}
	return 1;
...
Dialog:BisnisWithdraw(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		new bid = pData[playerid][pInBiz];
		...
		Dialog_Show(playerid, BisnisVault, DIALOG_STYLE_LIST,"Business Vault","Deposit\nWithdraw","Next","Back");
	return 1;
}

Dialog:BisnisDeposit(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		new bid = pData[playerid][pInBiz];
...
Dialog:BisnisBuyProd(playerid, response, listitem, inputtext[])
{
	static
    bizid = -1,
    price;
		...
		else if(bData[bizid][bType] == 3)
		{
			switch(listitem)
			{
				...
				case 1:
				{
					...
					Dialog_Show(playerid, ToyBuy, DIALOG_STYLE_LIST, ""RED_E"XGRP: "WHITE_E"Player Toys", string, "Select", "Cancel");
				}
				case 2:
				{
					GivePlayerMoneyEx(playerid, -price);
...
Dialog:BisnisEditProd(playerid, response, listitem, inputtext[])
{
	if(Player_OwnsBisnis(playerid, pData[playerid][pInBiz]))
	{
		if(response)
		{
			...
			Dialog_Show(playerid, BisnisPriceSet, DIALOG_STYLE_INPUT, "Business: Set Price", str, "Modify", "Back");
		}
		else
			return callcmd::bm(playerid, "\0");
	}
...
Dialog:BisnisPriceSet(playerid, response, listitem, inputtext[])
{
	static
    item[40];
...
Dialog:SellHouses(playerid, response, listitem, inputtext[])
{
	if(!response) return 1;
	new str[248];
	...
	Dialog_Show(playerid, SellHouse, DIALOG_STYLE_MSGBOX, "Sell House", str, "Sell", "Cancel");
	return 1;
}
...
Dialog:MyHouse(playerid, response, listitem, inputtext[])
{
	if(!response) return 1;
	SetPVarInt(playerid, "ClickedHouse", ReturnPlayerHousesID(playerid, (listitem + 1)));
	Dialog_Show(playerid, HouseInfo, DIALOG_STYLE_LIST, "{FF0000}SoI:RP {0000FF}Houses", "Show Information\nTrack House", "Select", "Cancel");
	return 1;
}
...
Dialog:HouseInfo(playerid, response, listitem, inputtext[])
{
	...
	switch(listitem)
	{
		case 0:
		{
			...
			Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, "House Info", line9, "Close","");
		}
		case 1:
		{
...
Dialog:HouseWeapon(playerid, response, listitem, inputtext[])
{
	...
	if(response)
	{
		switch (listitem)
		{
			case 0: 
			{
				...
				Dialog_Show(playerid, HouseWithdrawMoney, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
			}
			case 1: 
			{
				new str[128];
				format(str, sizeof(str), "Safe Balance: %s\n\nPlease enter how much money you wish to deposit into the safe:", FormatMoney(hData[houseid][hMoney]));
				Dialog_Show(playerid, HouseDepositMoney, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
			}
		}
	}
...
Dialog:HouseWithdrawMoney(playerid, response, listitem, inputtext[])
{
	new houseid = pData[playerid][pInHouse];
	if(!Player_OwnsHouse(playerid, pData[playerid][pInHouse])) return Error(playerid, "You don't own this house.");
...
Dialog:HouseDepositMoney(playerid, response, listitem, inputtext[])
{
	new houseid = pData[playerid][pInHouse];
	if(!Player_OwnsHouse(playerid, pData[playerid][pInHouse])) return Error(playerid, "You don't own this house.");
	if(response)
...
Dialog:FindVeh(playerid, response, listitem, inputtext[])
{
	if(response) 
	{
		Dialog_Show(playerid, TrackVeh, DIALOG_STYLE_INPUT, "Find Veh", "Enter your own vehicle id:", "Find", "Close");
	}
	return 1;
}

Dialog:TrackVeh(playerid, response, listitem, inputtext[])
{
	if(response) 
	{	
...
Dialog:GotoVeh(playerid, response, listitem, inputtext[])
{
	if(response) 
	{
		new Float:posisiX, Float:posisiY, Float:posisiZ,
...
Dialog:GetVeh(playerid, response, listitem, inputtext[])
{
	if(response) 
	{
		new Float:posisiX, Float:posisiY, Float:posisiZ,
...
Dialog:DeletevVeh(playerid, response, listitem, inputtext[])
{
	if(response) 
	{
...
Dialog:BuyPlateVeh(playerid, response, listitem, inputtext[])
{
	if(response) 
	{
		new carid = strval(inputtext);
...
Dialog:ToyScreen(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
		{
			case 0: //slot 1
			{
				...
				else
				{
					...
					Dialog_Show(playerid, EditToys, DIALOG_STYLE_LIST, ""RED_E"SOI:RP "WHITE_E"Player Toys", string, "Select", "Cancel");
				}
			}
			case 1: //slot 2
			{
				...
				else
				{
					...
					pToys[playerid][pData[playerid][toySelected]][toy_rx], pToys[playerid][pData[playerid][toySelected]][toy_ry], pToys[playerid][pData[playerid][toySelected]][toy_rz]);
					Dialog_Show(playerid, EditToys, DIALOG_STYLE_LIST, ""RED_E"SOI:RP "WHITE_E"Player Toys", string, "Select", "Cancel");
					//Dialog_Show(playerid, EditToys, DIALOG_STYLE_LIST, ""RED_E"SOI:RP "WHITE_E"Player Toys", ""dot"Edit Toy Position\n"dot"Change Bone\n"dot""GREY_E"Remove Toy\n"dot"Share Toy Pos", "Select", "Cancel");
				}
			}
			case 2: //slot 3
			{
	...
	if(response)
	{
		switch(listitem)
		{
			...
			case 1: // change bone
			{
				...
				Dialog_Show(playerid, ToyPosition, DIALOG_STYLE_LIST, ""RED_E"XGRP: "WHITE_E"Player Toys", finstring, "Select", "Cancel");
			}
			case 2: // remove toy
			{
			...
			case 4: //Pos X
			{
				...
				Dialog_Show(playerid, ToyPosX, DIALOG_STYLE_INPUT, "Toy PosX", mstr, "Edit", "Cancel");
			}
			case 5: //Pos Y
			{
				new mstr[128];
				...
				Dialog_Show(playerid, ToyPosX, DIALOG_STYLE_INPUT, "Toy PosY", mstr, "Edit", "Cancel");
			}
			case 6: //Pos Z
			{
				new mstr[128];
				...
				Dialog_Show(playerid, ToyPosZ, DIALOG_STYLE_INPUT, "Toy PosZ", mstr, "Edit", "Cancel");
			}
			case 7: //Pos RX
			{
				new mstr[128];
				...
				Dialog_Show(playerid, ToyPosRX, DIALOG_STYLE_INPUT, "Toy PosRX", mstr, "Edit", "Cancel");
			}
			case 8: //Pos RY
			{
				new mstr[128];
				...
				Dialog_Show(playerid, ToyPosRY, DIALOG_STYLE_INPUT, "Toy PosRY", mstr, "Edit", "Cancel");
			}
			case 9: //Pos RZ
			{
				new mstr[128];
				...
				Dialog_Show(playerid, ToyPosRZ, DIALOG_STYLE_INPUT, "Toy PosRZ", mstr, "Edit", "Cancel");
			}
		}
	}
	return 1;
}
Dialog:ToyPosition(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		listitem++;
...
Dialog:ToyPositionBuy(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		listitem++;
...
Dialog:ToyBuy(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:ToyVip(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:ToyPosX(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:ToyPosY(playerid, response, listitem, inputtext[])
{
	if(response)
	{
...
Dialog:ToyPosZ(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		new Float:posisi = floatstr(inputtext);
...
Dialog:ToyPosRX(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		new Float:posisi = floatstr(inputtext);
...
Dialog:ToyPosRY(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		new Float:posisi = floatstr(inputtext);
...
Dialog:ToyPosRZ(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		new Float:posisi = floatstr(inputtext);
...
Dialog:HelpScreen(playerid, response, listitem, inputtext[])
{
	if(!response) return 1;
	switch(listitem)
	{
		case 0:
		{
			...
			Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, ""RED_E"Player", str, "Close", "");
		}
		case 1:
		{
			new str[3500];
			strcat(str, ""LG_E"CHAT: /b /l /s /pm /togpm /w /o /me /ame /do /ado /try\n");
			Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, ""RED_E"Chat", str, "Close", "");
		}
		case 2:
		{
			new str[3500];
			...
			Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, ""RED_E"Vehicle", str, "Close", "");
		}
		case 3:
		{
			...
			Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, "Job Help", line1, "OK", "");
			return 1;
		}
		case 4:
		{
...
Dialog:GPSScreen(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
		{
			...
			case 9:
			{
				Dialog_Show(playerid, GPSJob, DIALOG_STYLE_LIST, "GPS JOB", "Taxi\nMechanic\nLumber Jack\nTrucker\nMiner\nProduction\nFarmer\nSweeper(Side Job)\nBus(Side Job)", "Select", "Close");
			}
		}
	}
	return 1;
...
Dialog:PayScreen(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		new mstr[128];
...
Dialog:EditBone(playerid, response, listitem, inputtext[])
{
	if(response)
	{
...
Dialog:FamilySafe(playerid, response, listitem, inputtext[])
{
	if(!response) return 1;
	...
	switch(listitem) 
	{
		...
		{
			//Marijuana
			Dialog_Show(playerid, FamilyMarijuana, DIALOG_STYLE_LIST, "Marijuana", "Withdraw from safe\nDeposit into safe", "Select", "Back");
		}
		case 2:
		{
			//Component
			Dialog_Show(playerid, FamilyComponent, DIALOG_STYLE_LIST, "Component", "Withdraw from safe\nDeposit into safe", "Select", "Back");
		}
		case 3:
		{
			//Material
			Dialog_Show(playerid, FamilyMaterial, DIALOG_STYLE_LIST, "Material", "Withdraw from safe\nDeposit into safe", "Select", "Back");
		}
		case 4:
		{
			//Money
			Dialog_Show(playerid, FamilyMoney, DIALOG_STYLE_LIST, "Money", "Withdraw from safe\nDeposit into safe", "Select", "Back");
		}
	}
	return 1;
}
...
Dialog:FamilyMarijuana(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		new fid = pData[playerid][pFamily];
		...
		if(response)
		{
			switch (listitem)
			{
				case 0: 
				{
					...
					Dialog_Show(playerid, FamilyWithdrawMarijuana, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
				}
				case 1: 
				{
					new str[128];
					format(str, sizeof(str), "Marijuana Balance: %d\n\nPlease enter how much marijuana you wish to deposit into the safe:", fData[fid][fMarijuana]);
					Dialog_Show(playerid, FamilyDepositMarijuana, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
				}
			}
		}
...
Dialog:FamilyWithdrawMarijuana(playerid, response, listitem, inputtext[])
{
	new fid = pData[playerid][pFamily];
	if(fid == -1) return Error(playerid, "You don't have family.");
...
Dialog:FamilyDepositMarijuana(playerid, response, listitem, inputtext[])
{
	new fid = pData[playerid][pFamily];
	if(fid == -1) return Error(playerid, "You don't have family.");
	if(response)
...
Dialog:FamilyComponent(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		new fid = pData[playerid][pFamily];
		...
		if(response)
		{
			switch (listitem)
			{
				case 0: 
				{
					...
					Dialog_Show(playerid, FamilyWithdrawComponent, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
				}
				case 1: 
				{
					new str[128];
					format(str, sizeof(str), "Component Balance: %d\n\nPlease enter how much component you wish to deposit into the safe:", fData[fid][fComponent]);
					Dialog_Show(playerid, FamilyDepositComponent, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
				}
			}
		}
		else callcmd::fsafe(playerid);
...
Dialog:FamilyWithdrawComponent(playerid, response, listitem, inputtext[])
{
		new fid = pData[playerid][pFamily];
		if(fid == -1) return Error(playerid, "You don't have family.");
...
Dialog:FamilyDepositComponent(playerid, response, listitem, inputtext[])
{
	new fid = pData[playerid][pFamily];
	if(fid == -1) return Error(playerid, "You don't have family.");
	if(response)
...
Dialog:FamilyMaterial(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		new fid = pData[playerid][pFamily];
		...
		if(response)
		{
			switch (listitem)
			{
				case 0: 
				{
					...
					Dialog_Show(playerid, FamilyWithdrawMaterial, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
				}
				case 1: 
				{
					new str[128];
					format(str, sizeof(str), "Material Balance: %d\n\nPlease enter how much material you wish to deposit into the safe:", fData[fid][fMaterial]);
					Dialog_Show(playerid, FamilyDepositMaterial, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
				}
			}
		}
		else callcmd::fsafe(playerid);
...
Dialog:FamilyWithdrawMaterial(playerid, response, listitem, inputtext[])
{
	new fid = pData[playerid][pFamily];
	if(fid == -1) return Error(playerid, "You don't have family.");
...
Dialog:FamilyDepositMaterial(playerid, response, listitem, inputtext[])
{
	new fid = pData[playerid][pFamily];
	if(fid == -1) return Error(playerid, "You don't have family.");
	if(response)
...
Dialog:FamilyMoney(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		new fid = pData[playerid][pFamily];
		...
		if(response)
		{
			switch (listitem)
			{
				case 0: 
				{
					...
					Dialog_Show(playerid, FamilyWithdrawMoney, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
				}
				case 1: 
				{
					new str[128];
					format(str, sizeof(str), "Money Balance: %s\n\nPlease enter how much money you wish to deposit into the safe:", FormatMoney(fData[fid][fMoney]));
					Dialog_Show(playerid, FamilyDepositMoney, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
				}
			}
		}
		else callcmd::fsafe(playerid);
...
Dialog:FamilyWithdrawMoney(playerid, response, listitem, inputtext[])
{
	new fid = pData[playerid][pFamily];
	if(fid == -1) return Error(playerid, "You don't have family.");
...
Dialog:FamilyDepositMoney(playerid, response, listitem, inputtext[])
{
	new fid = pData[playerid][pFamily];
	if(fid == -1) return Error(playerid, "You don't have family.");
	if(response)
	...
	if(response)
	{
		switch(listitem)
		{
			...
			case 1:
			{
				...
				format(lstr, sizeof(lstr), "%s\n", lstr);
				Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_TABLIST_HEADERS, "Family Online", lstr, "Close", "");
...
Dialog:LockerVIP(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch (listitem) 
		{
			...
			case 3:
			{
				...
				Dialog_Show(playerid, ToyVip, DIALOG_STYLE_LIST, ""RED_E"SoI:RP "WHITE_E"VIP Toys", string, "Select", "Cancel");
			}
		}
	}
	return 1;
	...
	if(response)
	{
		switch (listitem) 
		{
			...
			case 3:
			{
				...
				Dialog_Show(playerid, WesponSANEW, DIALOG_STYLE_LIST, "SAPD Weapons", "CAMERA\nSPRAYCAN\nPARACHUTE\nNITE STICK\nKNIFE\nCOLT45", "Pilih", "Batal");
			}
			case 4:
			{
				if(pData[playerid][pOnDuty] <= 0)
...
Dialog:ServiceScreen(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
		{
			...
			case 2:
			{
				...
				IsPlayerInRangeOfPoint(playerid, 15.0, 298.59, -248.54, 1.60) || IsPlayerInRangeOfPoint(playerid, 10.0, 266.20, -253.32, 1.59))
				{
					...
					if(GetNearestVehicleToPlayer(playerid, 3.8, false) == pData[playerid][pMechVeh])
					{
						...
						Dialog_Show(playerid, ServiceColor1, DIALOG_STYLE_INPUT, "Color ID 1", "Enter the color id 1:(0 - 255)", "Next", "Close");
					}
					else
					{
						KillTimer(pData[playerid][pMechanic]);
			...
			case 3:
			{
				...
				IsPlayerInRangeOfPoint(playerid, 15.0, 298.59, -248.54, 1.60) || IsPlayerInRangeOfPoint(playerid, 10.0, 266.20, -253.32, 1.59))
				{
					...
					if(GetNearestVehicleToPlayer(playerid, 3.8, false) == pData[playerid][pMechVeh])
					{
						...
						Dialog_Show(playerid, ServicePaintJob, DIALOG_STYLE_INPUT, "Paintjob", "Enter the vehicle paintjob id:(0 - 2 | 3 - Remove paintJob)", "Paintjob", "Close");
					}
					else
					{
						KillTimer(pData[playerid][pMechanic]);
			...
			case 4:
			{
				...
				IsPlayerInRangeOfPoint(playerid, 15.0, 298.59, -248.54, 1.60) || IsPlayerInRangeOfPoint(playerid, 10.0, 266.20, -253.32, 1.59))
				{
					...
					if(GetNearestVehicleToPlayer(playerid, 3.8, false) == pData[playerid][pMechVeh])
					{
						...
						Dialog_Show(playerid, ServiceWheels, DIALOG_STYLE_LIST, "Wheels", "Offroad\nMega\nWires\nTwist\nGrove\nImport\nAtomic\nAhab\nVirtual\nAccess\nTrance\nShadow\nRimshine\nClassic\nCutter\nSwitch\nDollar", "Confirm", "back");
					}
					else
					{
						KillTimer(pData[playerid][pMechanic]);
			...
			case 6:
			{
				...
				IsPlayerInRangeOfPoint(playerid, 15.0, 298.59, -248.54, 1.60) || IsPlayerInRangeOfPoint(playerid, 10.0, 266.20, -253.32, 1.59))
				{
					...
					{
						if(pData[playerid][pComponent] < 70) return Error(playerid, "Component anda kurang!");
						Dialog_Show(playerid, ServiceHoods, DIALOG_STYLE_LIST, "Hoods", "Fury\nChamp\nRace\nWorx\n", "Confirm", "back");
					}
					else
					{
						KillTimer(pData[playerid][pMechanic]);
			...
			case 7:
			{
				...
				IsPlayerInRangeOfPoint(playerid, 15.0, 298.59, -248.54, 1.60) || IsPlayerInRangeOfPoint(playerid, 10.0, 266.20, -253.32, 1.59))
				{
					...
					{
						if(pData[playerid][pComponent] < 70) return Error(playerid, "Component anda kurang!");
						Dialog_Show(playerid, ServiceVents, DIALOG_STYLE_LIST, "Vents", "Oval\nSquare\n", "Confirm", "back");
					}
					else
					{
						KillTimer(pData[playerid][pMechanic]);
			...
			case 8:
			{
				...
				IsPlayerInRangeOfPoint(playerid, 15.0, 298.59, -248.54, 1.60) || IsPlayerInRangeOfPoint(playerid, 10.0, 266.20, -253.32, 1.59))
				{
					...
					if(GetNearestVehicleToPlayer(playerid, 3.8, false) == pData[playerid][pMechVeh])
					{
						...
						Dialog_Show(playerid, ServiceLights, DIALOG_STYLE_LIST, "Lights", "Round\nSquare\n", "Confirm", "back");
					}
					else
					{
						KillTimer(pData[playerid][pMechanic]);
			...
			case 10:
			{
				...
				IsPlayerInRangeOfPoint(playerid, 15.0, 298.59, -248.54, 1.60) || IsPlayerInRangeOfPoint(playerid, 10.0, 266.20, -253.32, 1.59))
				{
					...
					{
						if(pData[playerid][pComponent] < 100) return Error(playerid, "Component anda kurang!");
						Dialog_Show(playerid, ServiceFrontBumpers, DIALOG_STYLE_LIST, "Front bumpers", "Wheel Arc. Alien Bumper\nWheel Arc. X-Flow Bumper\nLow co. Chromer Bumper\nLow co. Slamin Bumper", "Confirm", "back");
					}
					else
					{
						KillTimer(pData[playerid][pMechanic]);
			...
			case 11:
			{
				...
				IsPlayerInRangeOfPoint(playerid, 15.0, 298.59, -248.54, 1.60) || IsPlayerInRangeOfPoint(playerid, 10.0, 266.20, -253.32, 1.59))
				{
					...
					{
						if(pData[playerid][pComponent] < 100) return Error(playerid, "Component anda kurang!");
						Dialog_Show(playerid, ServiceRearBumpers, DIALOG_STYLE_LIST, "Rear bumpers", "Wheel Arc. Alien Bumper\nWheel Arc. X-Flow Bumper\nLow co. Chromer Bumper\nLow co. Slamin Bumper", "Confirm", "back");
					}
					else
					{
						KillTimer(pData[playerid][pMechanic]);
			...
			case 12:
			{
				...
				IsPlayerInRangeOfPoint(playerid, 15.0, 298.59, -248.54, 1.60) || IsPlayerInRangeOfPoint(playerid, 10.0, 266.20, -253.32, 1.59))
				{
					...
					{
						if(pData[playerid][pComponent] < 70) return Error(playerid, "Component anda kurang!");
						Dialog_Show(playerid, ServiceRoofs, DIALOG_STYLE_LIST, "Roofs", "Wheel Arc. Alien\nWheel Arc. X-Flow\nLow Co. Hardtop Roof\nLow Co. Softtop Roof\nTransfender Roof Scoop", "Confirm", "back");
					}
					else
					{
						KillTimer(pData[playerid][pMechanic]);
			...
			case 13:
			{
				...
				IsPlayerInRangeOfPoint(playerid, 15.0, 298.59, -248.54, 1.60) || IsPlayerInRangeOfPoint(playerid, 10.0, 266.20, -253.32, 1.59))
				{
					...
					if(GetNearestVehicleToPlayer(playerid, 3.8, false) == pData[playerid][pMechVeh])
					{
						...
						Dialog_Show(playerid, ServiceSideSkirts, DIALOG_STYLE_LIST, "Side skirts", "Wheel Arc. Alien Side Skirt\nWheel Arc. X-Flow Side Skirt\nLocos Chrome Strip\nLocos Chrome Flames\nLocos Chrome Arches \nLocos Chrome Trim\nLocos Wheelcovers\nTransfender Side Skirt", "Confirm", "back");
					}
					else
					{
						KillTimer(pData[playerid][pMechanic]);
			...
			case 14:
			{
				...
				IsPlayerInRangeOfPoint(playerid, 15.0, 298.59, -248.54, 1.60) || IsPlayerInRangeOfPoint(playerid, 10.0, 266.20, -253.32, 1.59))
				{
					...
					{
						if(pData[playerid][pComponent] < 50) return Error(playerid, "Component anda kurang!");
						Dialog_Show(playerid, ServiceBullbars, DIALOG_STYLE_LIST, "Bull bars", "Locos Chrome Grill\nLocos Chrome Bars\nLocos Chrome Lights \nLocos Chrome Bullbar", "Confirm", "back");
					}
					else
					{
						KillTimer(pData[playerid][pMechanic]);
			...
			case 20:
			{
				...
				IsPlayerInRangeOfPoint(playerid, 15.0, 298.59, -248.54, 1.60) || IsPlayerInRangeOfPoint(playerid, 10.0, 266.20, -253.32, 1.59))
				{
					...
					if(GetNearestVehicleToPlayer(playerid, 3.8, false) == pData[playerid][pMechVeh])
					{
						...
						Dialog_Show(playerid, ServiceNeon,DIALOG_STYLE_LIST,"Neon","RED\nBLUE\nGREEN\nYELLOW\nPINK\nWHITE\nREMOVE","Choose","back");
					}
					else
					{
						KillTimer(pData[playerid][pMechanic]);
...
Dialog:ServiceColor1(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		if(pData[playerid][pMechColor1] < 0 || pData[playerid][pMechColor1] > 255)
			return Dialog_Show(playerid, DIALOG_SERVICE_COLOR, DIALOG_STYLE_INPUT, "Color ID 1", "Enter the color id 1:(0 - 255)", "Next", "Close");
		
		Dialog_Show(playerid, ServiceColor2, DIALOG_STYLE_INPUT, "Color ID 2", "Enter the color id 2:(0 - 255)", "Next", "Close");
	}
	return 1;
}
Dialog:ServiceColor2(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		if(pData[playerid][pMechColor2] < 0 || pData[playerid][pMechColor2] > 255)
			return Dialog_Show(playerid, ServiceColor2, DIALOG_STYLE_INPUT, "Color ID 2", "Enter the color id 2:(0 - 255)", "Next", "Close");
...
Dialog:ServicePaintJob(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		pData[playerid][pMechColor1] = floatround(strval(inputtext));
		...
		if(pData[playerid][pMechColor1] < 0 || pData[playerid][pMechColor1] > 3)
			return Dialog_Show(playerid, ServicePaintJob, DIALOG_STYLE_INPUT, "Paintjob", "Enter the vehicle paintjob id:(0 - 2 | 3 - Remove paintJob)", "Paintjob", "Close");
...
Dialog:ServiceWheels(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:ServiceSpoiler(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:ServiceHoods(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:ServiceVents(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:ServiceLights(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:ServiceExhausts(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:ServiceFrontBumpers(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:ServiceRearBumpers(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:ServiceRoofs(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:ServiceSideSkirts(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:ServiceBullbars(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:ServiceNeon(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:ArmsGun(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:PlantScreen(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:HaulingScreen(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, "Hauling Info", line9, "Close","");
	}
	return 1;
}
Dialog:VerifyCodeScreen(playerid, response, listitem, inputtext[])
{
	if(!response) return Kick(playerid);
	if(response)
	{
		...
		if(isnull(inputtext))
		{
			...
			return Dialog_Show(playerid, VerifyCodeScreen, DIALOG_STYLE_INPUT, "Verify Account", str, "Input", "Cancel");
		}
		...
		{
			new lstring[512];
			format(lstring, sizeof lstring, "{ffffff}Welcome to {15D4ED}Century City Roleplay\n{ffffff}UCP: {15D4ED}%s\n{ffffff}Password: \nSilahkan buat password baru kamu!:", pData[playerid][pUCP]);
			Dialog_Show(playerid, RegisterScreen, DIALOG_STYLE_PASSWORD, "Registration IndoLast", lstring, "Register", "Quit");
			return 1;
		}
...
Dialog:RestockBisnis(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		//SetPlayerCheckpoint(playerid, -279.67, -2148.42, 28.54, 3.5);
		Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, "Mission Info", line9, "Close","");
	}
	return 1;
}
Dialog:ProductBisnis(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		new amount = floatround(strval(inputtext));
...
Dialog:GasOilBisnis(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		new amount = floatround(strval(inputtext));
...
Dialog:MaterialScreen(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		new amount = floatround(strval(inputtext));
...
Dialog:ComponentScreen(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		new amount = floatround(strval(inputtext));
...
Dialog:DrugsScreen(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		new amount = floatround(strval(inputtext));
...
Dialog:FoodScreen(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
		{
			case 0:
			{
				...
				Dialog_Show(playerid, FoodBuy, DIALOG_STYLE_INPUT, "Buy Food", mstr, "Buy", "Cancel");
			}
			case 1:
			{
				//buy seed
				...
				Dialog_Show(playerid, SeedBuy, DIALOG_STYLE_INPUT, "Buy Seed", mstr, "Buy", "Cancel");
			}
		}
	}
	return 1;
}
Dialog:FoodBuy(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		new amount = floatround(strval(inputtext));
...
Dialog:SeedBuy(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		new amount = floatround(strval(inputtext));
...
Dialog:EditPriceScreen(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
		{
			case 0:
			{
				...
				Dialog_Show(playerid, EditPriceScreen1, DIALOG_STYLE_INPUT, "Price 1", mstr, "Edit", "Cancel");
			}
			case 1:
			{
				new mstr[128];
				format(mstr, sizeof(mstr), ""WHITE_E"Masukan harga Snack(1 - 500):\nPrice 2(Snack): "GREEN_E"%s", FormatMoney(pData[playerid][pPrice2]));
				Dialog_Show(playerid, EditPriceScreen2, DIALOG_STYLE_INPUT, "Price 2", mstr, "Edit", "Cancel");
			}
			case 2:
			{
				new mstr[128];
				...
				Dialog_Show(playerid, EditPriceScreen3, DIALOG_STYLE_INPUT, "Price 3", mstr, "Edit", "Cancel");
			}
			case 3:
			{
				new mstr[128];
				format(mstr, sizeof(mstr), ""WHITE_E"Masukan harga Hotdog(1 - 500):\nPrice 4(Hotdog): "GREEN_E"%s", FormatMoney(pData[playerid][pPrice4]));
				Dialog_Show(playerid, EditPriceScreen4, DIALOG_STYLE_INPUT, "Price 4", mstr, "Edit", "Cancel");
			}
		}
	}
	return 1;
}
Dialog:EditPriceScreen1(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		new amount = floatround(strval(inputtext));
...
Dialog:EditPriceScreen2(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		new amount = floatround(strval(inputtext));
...
Dialog:EditPriceScreen3(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		new amount = floatround(strval(inputtext));
...
Dialog:EditPriceScreen4(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		new amount = floatround(strval(inputtext));
...
Dialog:OfferScreen(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:ApotekScreen(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:ATMScreen(playerid, response, listitem, inputtext[])
{
	if(!response) return 1;
	switch(listitem)
	{
		case 0: // Check Balance
		{
			...
			Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, ""LB_E"Bank", mstr, "Close", "");
		}
		case 1: // Withdraw
		{
			new mstr[128];
			format(mstr, sizeof(mstr), ""WHITE_E"My Balance: "LB_E"%s", FormatMoney(pData[playerid][pBankMoney]));
			Dialog_Show(playerid, ATMWithdraw, DIALOG_STYLE_LIST, mstr, "$50\n$200\n$500\n$1.000\n$5.000", "Withdraw", "Cancel");
		}
		case 2: // Transfer Money
		{
			Dialog_Show(playerid, BANKRekening, DIALOG_STYLE_INPUT, ""LB_E"Bank", "Masukan jumlah uang:", "Transfer", "Cancel");
		}
		case 3: //Paycheck
		{
...
Dialog:BANKScreen(playerid, response, listitem, inputtext[])
{
	if(!response) return true;
	switch(listitem)
	{
		case 0: // Deposit
		{
			...
			Dialog_Show(playerid, BANKDeposit, DIALOG_STYLE_INPUT, ""LB_E"Bank", mstr, "Deposit", "Cancel");
		}
		case 1: // Withdraw
		{
			new mstr[512];
			...
			Dialog_Show(playerid, BANKWithdraw, DIALOG_STYLE_INPUT, ""LB_E"Bank", mstr, "Withdraw", "Cancel");
		}
...
Dialog:BANKDeposit(playerid, response, listitem, inputtext[])
{
	...
	else
	{
		...
		Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, ""ORANGE_E"XGRP: "LB_E"Bank", lstr, "Close", "");
	}
	return 1;
}
...
Dialog:BANKRekening(playerid, response, listitem, inputtext[])
{
	if(!response) return true;
	new amount = floatround(strval(inputtext));
	...
	else
	{
		...
		Dialog_Show(playerid, BANKTransfer, DIALOG_STYLE_INPUT, ""LB_E"Bank", "Masukan nomor rekening target:", "Transfer", "Cancel");
	}
	return 1;
}
...
Dialog:BANKConfirm(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		Dialog_Show(playerid, BANKSukses, DIALOG_STYLE_MSGBOX, ""LB_E"Transfer Sukses", mstr, "Sukses", "");
	}
	return 1;
}
Dialog:BANKSukses(playerid, response, listitem, inputtext[])
{
	if(response)
	{
...
Dialog:ReportsScreen(playerid, response, listitem, inputtext[])
{
	if(response) 
	{
		...
		Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX,tstr,lstr,"Close","");
	}
	return 1;
}
Dialog:BuyPV(playerid, response, listitem, inputtext[])
{
...
Dialog:BuyVIPPV(playerid, response, listitem, inputtext[])
{
	new vehicleid = GetPlayerVehicleID(playerid);
	if(response)
	{
...
Dialog:BuyPVCP(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
		{
			case 0:
			{
				...
				Dialog_Show(playerid, BuyPVCPBikes, DIALOG_STYLE_LIST, "Bikes", str, "Buy", "Close");
			}
			case 1:
			{
				//Cars
				...
				Dialog_Show(playerid, BuyPVCPCars, DIALOG_STYLE_LIST, "Cars", str, "Buy", "Close");
			}
			case 2:
			{
				//Unique Cars
				...
				Dialog_Show(playerid, BuyPVCPUCars, DIALOG_STYLE_LIST, "Unique Cars", str, "Buy", "Close");
			}
			case 3:
			{
				//Job Cars
				...
				Dialog_Show(playerid, BuyPVCPJobCars, DIALOG_STYLE_LIST, "Job Cars", str, "Buy", "Close");
			}
			case 4:
			{
				// VIP Cars
				...
				Dialog_Show(playerid, BuyPVCPVIPCras, DIALOG_STYLE_LIST, "Vip Cars", str, "Buy", "Close");
			}
		}
	}
	return 1;
}
Dialog:BuyPVCPBikes(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
		{
			case 0:
			{
				...
				Dialog_Show(playerid, BuyPVCPVIPConfirm, DIALOG_STYLE_MSGBOX, "Private Vehicles", tstr, "Buy", "Batal");
			}
			case 1:
			{
				new modelid = 509;
...
Dialog:BuyPVCPCars(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
		{
			case 0:
			{
				...
				pData[playerid][pBuyPvModel] = modelid;
				format(tstr, sizeof(tstr), ""WHITE_E"Anda akan membeli kendaraan "PINK_E"%s "WHITE_E"dengan harga "LG_E"%s", GetVehicleModelName(modelid), FormatMoney(price));
				Dialog_Show(playerid, BuyPVCPConfirm, DIALOG_STYLE_MSGBOX, "Private Vehicles", tstr, "Buy", "Batal");
			}
			case 1:
			{
				new modelid = 412;
...
Dialog:BuyPVCPUCars(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:BuyPVCPJobCars(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:BuyPVCPVIPCras(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
...
Dialog:RentJobCars(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
		{
			case 0:
			{
				...
				Dialog_Show(playerid, RentJobCarsConfirm, DIALOG_STYLE_MSGBOX, "Rental Vehicles", tstr, "Rental", "Batal");
			}
			case 1:
			{
				new modelid = 455;
...
Dialog:RentJobCarsConfirm(playerid, response, listitem, inputtext[])
{
...
Dialog:BuyPVCPConfirm(playerid, response, listitem, inputtext[])
{
	new modelid = pData[playerid][pBuyPvModel];
	if(response)
	{
...
Dialog:BuyPVCPVIPConfirm(playerid, response, listitem, inputtext[])
{
	new modelid = pData[playerid][pBuyPvModel];
	if(response)
	{
...
Dialog:PayCheckScreen(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		if(rows) 
		{
			...
			Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, "Paycheck", list, "Close", "");
...
Dialog:SweeperScreen(playerid, response, listitem, inputtext[])
{
	new vehicleid = GetPlayerVehicleID(playerid);
	if(response)
	{
...
Dialog:BusScreen(playerid, response, listitem, inputtext[])
{
	new vehicleid = GetPlayerVehicleID(playerid);
	if(response)
	{
