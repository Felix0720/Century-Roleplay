...
Family_WeaponStorage(playerid, fid)
{
    ...
    Dialog_Show(playerid, FamilyWeapon, DIALOG_STYLE_LIST, "Weapon Storage", string, "Select", "Cancel");
    return 1;
}
...
CMD:fsafe(playerid)
{
    ...
    {
     	Dialog_Show(playerid, FamilySafe, DIALOG_STYLE_LIST, "Family SAFE", "Storage\nMarijuana\nComponent\nMaterial\nMoney", "Select", "Cancel");
    }
 	else
   	{
 	...
 	if(rows)
  	{
		...
		Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, "Family Info", string, "Okay", "");
	}
}
 	...
 	if(rows)
  	{
		...
		Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, "Family Member", query, "Okay", "");
	}
}