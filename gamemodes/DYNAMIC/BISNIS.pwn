...
Bisnis_BuyMenu(playerid, bizid)
{
    ...
    switch(bData[bizid][bType])
    {
        case 1:
        {
            ...
            Dialog_Show(playerid, BisnisBuyProd, DIALOG_STYLE_LIST, bData[bizid][bName], string, "Buy", "Cancel");
        }
        case 2:
        {
...
Bisnis_ProductMenu(playerid, bizid)
{
    ...
    switch (bData[bizid][bType])
    {
        case 1:
        {
            ...
            Dialog_Show(playerid, BisnisEditProd, DIALOG_STYLE_LIST, "Business: Modify Item", string, "Modify", "Cancel");
        }
        case 2:
        {
            format(string, sizeof(string), "Snack - %s\nSprunk - %s\nGas Fuel - %s\nBandage - %s\nGPS - %s\nPhone - %s\nPhone Credit - %s\nPhone Book - %s\nWalkie Talkie - %s",
...
Bisnis_Refresh(id)
{
    if(id != -1)
    {
		...
		{
			format(string, sizeof(string), ""WHITE_E"--===["GREEN_E"Bisnis Info"WHITE_E"]===--\n"WHITE_E"ID: %d\n"WHITE_E"Business Type: "LB_E"%s\n"WHITE_E"Business Name: "ORANGE_E"%s\n"WHITE_E"Business Owner: "LIME_E"%s\n"WHITE_E"Press '"LRED_E"Enter/Otot F"WHITE_E"' to enter\n"WHITE_E"--===["GREEN_E"Business Info"WHITE_E"]===--", id, type, bData[id][bName], bData[id][bOwner]);
			bData[id][bPickup] = CreateDynamicPickup(19133, 23, bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ]+0.2, 0, 0, _, 8.0);
        }
        else
        {
            format(string, sizeof(string), ""WHITE_E"--===["RED_E"Business For Sale"WHITE_E"]===--\n"WHITE_E"ID: %d\n"WHITE_E"Business Location: "ORANGE_E"%s\n"WHITE_E"Business Price: "GREEN_E"%s\n"WHITE_E"Business Type: "LB_E"%s\n"YELLOW_E"Type /buy to purchase\n"WHITE_E"--==["RED_E"Business For Sale"WHITE_E"]==--", id, GetLocation(bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ]), FormatMoney(bData[id][bPrice]), type);
            bData[id][bPickup] = CreateDynamicPickup(19133, 23, bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ]+0.2, 0, 0, _, 8.0);
        }
...
CMD:sellbisnis(playerid, params[])
{
	...
	Dialog_Show(playerid, SellBisniss, DIALOG_STYLE_LIST, "Sell Bisnis", CMDSString, "Sell", "Cancel");
	return 1;
}
...
CMD:mybis(playerid)
{
	...
	Dialog_Show(playerid, MyBisnis, DIALOG_STYLE_LIST, "{FF0000}SoI:RP {0000FF}Bisnis", CMDSString, "Select", "Cancel");
	return 1;
}
...
CMD:bm(playerid, params[])
{
    ...
    Dialog_Show(playerid, BisnisMenu, DIALOG_STYLE_LIST, "Bisnis Menu","Bisnis Info\nChange Name\nBisnis Vault\nProduct Menu\nRequest Restock","Next","Close");
    return 1;
}
