...
CMD:credits(playerid)
{
	...
	format(line1, sizeof(line1), "%s%s", line3, line2);
   	Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, ""ORANGE_E"Sunrise-RP: "WHITE_E"Server Credits", line1, "OK", "");
	return 1;
}
...
CMD:vip(playerid)
{
	...
	Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, ""ORANGE_E"Sunrise:RP "PINK_E"VIP SYSTEM", longstr2, "Close", "");
	return 1;
}
...
CMD:donate(playerid)
{
	...
	Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, ""ORANGE_E"LSTAR: "WHITE_E"DONATE LIST", line3, "Okay", "");
	return 1;
}
...
CMD:changepass(playerid)
{
	...
	Dialog_Show(playerid, PasswordScreen, DIALOG_STYLE_INPUT, ""WHITE_E"Change your password", "Insert your new password to change!", "Change", "Exit");
...
CMD:gps(playerid, params[])
{
	...
	Dialog_Show(playerid, DIALOG_GPS, DIALOG_STYLE_LIST, "GPS Menu", "Disable GPS\nMy House\nMy Bisnis\nMy Vehicle\nMy Mission(Trucker)\nMy Hauling(Trucker)\nLos Santos City Hall\nBank Los Santos\nShowroom/Dealer\nJobs", "Select", "Close");
	return 1;
}
...
/*CMD:salary(playerid, params[])
{
	...
	if(rows) 
	{
		...
		Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_TABLIST_HEADERS, "Salary Details", str, "Close", "");
	}
	else 
	{
		Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, "Notice", "Kamu tidak memiliki salary saat ini!", "Ok", "");
	}
	cache_delete(result);
	return 1;
}*/
...
CMD:time(playerid)
{
	...
	format(line2, sizeof(line2), ""WHITE_E"Paycheck Time: "YELLOW_E"%d remaining\n"WHITE_E"Job Delay Time: "RED_E"%d second\n"WHITE_E"Side Job Delay Time: "RED_E"%d second\n"WHITE_E"Plant Time(Farmer): "RED_E"%d second\n"WHITE_E"Arrest Time: "RED_E"%d second\n"WHITE_E"Jail Time: "RED_E"%d second\n", paycheck, pData[playerid][pJobTime], pData[playerid][pSideJobTime], pData[playerid][pPlantTime], pData[playerid][pArrestTime], pData[playerid][pJailTime]);
   	Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, ""ORANGE_E"Sunrise: "WHITE_E"My Time", line2, "Okay", "");
	return 1;
}
...
CMD:newidcard(playerid, params[])
{
	...
	Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, "ID-Card", mstr, "Tutup", "");
...
CMD:newage(playerid, params[])
{
	...
	Dialog_Show(playerid, ChangeAge, DIALOG_STYLE_INPUT, "Tanggal Lahir", "Masukan tanggal lahir (Tgl/Bulan/Tahun): 15/04/1998", "Change", "Cancel");
	return 1;
}
...
CMD:newdrivelic(playerid, params[])
{
	...
	if(pData[playerid][pGender] == 1) { sext = "Laki-Laki"; } else { sext = "Perempuan"; }
	format(mstr, sizeof(mstr), "{FFFFFF}Nama: %s\nNegara: San Andreas\nTgl Lahir: %s\nJenis Kelamin: %s\nBerlaku hingga 14 hari!", pData[playerid][pName], pData[playerid][pAge], sext);
	Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, "Driving License", mstr, "Tutup", "");
...
/*CMD:buyplate(playerid, params[])
{
	...
	if(found)
		Dialog_Show(playerid, DIALOG_BUYPLATE, DIALOG_STYLE_TABLIST_HEADERS, "Vehicles Plate", msg2, "Buy", "Close");
	else
		Dialog_Show(playerid, UnusedScreen, DIALOG_STYLE_MSGBOX, "Vehicles Plate", "Anda tidak memeliki kendaraan", "Close", "");
	...
	foreach(new i : PVehicles)
	{
		if(vehid == pvData[i][cVeh])
		{
			if(pvData[i][cOwner] == pData[playerid][pID])
			{
				...
				format(pvData[i][cPlate], 32, "Sunrise-%d", rand);
...
CMD:bank(playerid, params[])
{
	...
	Dialog_Show(playerid, BankScreen, DIALOG_STYLE_LIST, tstr, "Deposit Money\nWithdraw Money\nCheck Balance\nTransfer Money\nSign Paycheck", "Select", "Cancel");
	return 1;
}
...
CMD:pay(playerid, params[])
{
	...
	Dialog_Show(playerid, DIALOG_PAY, DIALOG_STYLE_MSGBOX, ""GREEN_E"Send Money", mstr, "Send", "Cancel");
...
CMD:settings(playerid)
{
	...
	Dialog_Show(playerid, SettingsScreen, DIALOG_STYLE_LIST, "Settings", str, "Set", "Close");
	return 1;
}
...
CMD:buy(playerid, params[])
{
	...
	if(IsPlayerInRangeOfPoint(playerid, 3.5, -279.67, -2148.42, 28.54))
	{
		if(pData[playerid][pJob] == 4 || pData[playerid][pJob2] == 4)
		{
			if(GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
			{
				...
				Dialog_Show(playerid, DIALOG_PRODUCT, DIALOG_STYLE_INPUT, "Buy Product", mstr, "Buy", "Cancel");
			}
	...
	if(IsPlayerInRangeOfPoint(playerid, 3.5, 336.70, 895.54, 20.40))
	{
		if(pData[playerid][pJob] == 4 || pData[playerid][pJob2] == 4)
		{
			if(GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
			{
				...
				Dialog_Show(playerid, DIALOG_GASOIL, DIALOG_STYLE_INPUT, "Buy GasOil", mstr, "Buy", "Cancel");
			}
			else return Error(playerid, "You are not in vehicle trucker.");
		}
		else return Error(playerid, "You are not trucker job.");
	...
	if(IsPlayerInRangeOfPoint(playerid, 2.5, -258.54, -2189.92, 28.97))
	{
		...
		Dialog_Show(playerid, DIALOG_MATERIAL, DIALOG_STYLE_INPUT, "Buy Material", mstr, "Buy", "Cancel");
	}
	//Component
	if(IsPlayerInRangeOfPoint(playerid, 2.5, 315.07, 926.53, 20.46))
	{
		...
		Dialog_Show(playerid, DIALOG_COMPONENT, DIALOG_STYLE_INPUT, "Buy Component", mstr, "Buy", "Cancel");
	}
	//Apotek
	if(IsPlayerInRangeOfPoint(playerid, 2.5, 1435.34, -23.91, 1000.92))
	{
		...
		Dialog_Show(playerid, DIALOG_APOTEK, DIALOG_STYLE_TABLIST_HEADERS, "Apotek", mstr, "Buy", "Cancel");
	}
	//Food and Seed
	if(IsPlayerInRangeOfPoint(playerid, 2.5, -381.44, -1426.13, 25.93))
	{
		...
		Dialog_Show(playerid, DIALOG_FOOD, DIALOG_STYLE_TABLIST_HEADERS, "Food", mstr, "Buy", "Cancel");
	}
	//Drugs
	if(IsPlayerInRangeOfPoint(playerid, 2.5, -3811.65, 1313.72, 71.42))
	{
		...
		Dialog_Show(playerid, DIALOG_DRUGS, DIALOG_STYLE_INPUT, "Buy Drugs", mstr, "Buy", "Cancel");
	}
	//Buy House
	foreach(new hid : Houses)
	{
